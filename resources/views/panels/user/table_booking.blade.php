        ...
        <div class="row m-t30">
            ...
            <div class="col-sm-9 padding-right">
                <div class="features_items"><!--features_items-->
                    <h2 class="title text-center">Table Booking</h2>
                        ...
                        <div class="col-sm-12">
							...
							<table id="datatable" class="table table-striped table-bordered bulk_action">
								...
								<tbody>
									@foreach($table_bookings as $table_booking)
										<tr>
											<td>{{ $table_booking->order_date }}</td>
											<td>{{ $table_booking->booked_date }}</td>
											<td>{{ $table_booking->booked_time }}</td>
											<td>{{ $table_booking->listing->title }}</td>
											<td>
												@php
													$address = $table_booking->listing->address1;
													$city = $table_booking->listing->listing_city->name;
													$phone = $table_booking->listing->phoneno;
													if($table_booking->listing->address2)
														$address .= ', ' . $table_booking->listing->address2;
													echo $address, ', ' , $city . '. <br />', $phone;
												@endphp
											</td>
											<td>{{ $table_booking->status }}</td>
											<td><a href="{{ url($table_booking->listing->slug) }}" target="_blank">View</a></td>
										</tr> 
									@endforeach
