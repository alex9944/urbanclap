            ...
            <div class="row">
					...
					<!-- <div class="col-md-12 col-xs-12">
					 ...
					 <div class="col-md-5 col-xs-12">
								<div class="x_panel">
								<form name="actionForm" action="{{url('admin/questions/destroy')}}" method="post" onsubmit="return deleteConfirm();"/> 
									<h2>All Questions <span class="pull-right"><a href="{{url('admin/questions')}}" class="btn btn-success btn-xs"><i class="fa fa-plus"></i> Add </a>  
					...
					@foreach ($questions as $question)
                        <tr class="rm{{ $question->id }}">
                          <td>
						  <input type="checkbox" name="selected_id[]" class="checkbox" value="{{ $question->id }}"/>				 	  
						  </td>
                          <td>{{ $question->id }}</td>
                          <td>{{ $question->service->id }}</td>
                          <td>{{ $question->service->name }}</td>
                         <td>
						 <!--<a href="#" class="btn btn-primary btn-xs"><i class="fa fa-folder"></i> View </a>-->
                            <a href="javascript:void(0);" class="edit_blog btn btn-info btn-xs" data-id="{{ $question->id }}" ><i class="fa fa-pencil"></i></a> 
                            <a href="javascript:void(0);" class="btn btn-danger btn-xs delete_blog" data-id="{{ $question->id }}"><i class="fa fa-trash-o"></i> </a></td>
                        </tr> 
					@endforeach
					...
					<div class="x_panel" id="edit_div">
                        ...
                        <h2><span id="add_div_label">{{$add}}</span> Questions </h2>
						<div class="x_title">
						</div>	
                                ...
                                <div class="form-group ">
                                    ...
                                    <div class="col-sm-10">
                                        ...
                                        <table class="table table-striped table-bordered">
                                              ...
                                              <tr style="">
                                                 <td>{{$question_arr['name']}}</td>
                                                 <td><input type="radio" id="st_qn_st_yes_{{$question_arr['slug']}}" name="standared_qtn_status_{{$question_arr['slug']}}" value="yes">Yes <input type="radio" name="standared_qtn_status_{{$question_arr['slug']}}" value="no" checked>No</td>
                                                 <td><input type="radio" id="st_qn_ma_yes_{{$question_arr['slug']}}" value="yes" name="standared_qtn_mandate_{{$question_arr['slug']}}">Yes &nbsp;&nbsp;<input type="radio" value="no" name="standared_qtn_mandate_{{$question_arr['slug']}}" checked>No</td>
                                              </tr>
                                              @endforeach
                                        </table>
                                     </div>
...
$(document).ready(function() {
									...
									+'<td>'
										+'<div class="col-sm-5">'
											...
											+'<select class="question_input form-control" name="question_input'+ s1x +'" data-sno="'+ s1x +'">'
											  ...
											  +'<option value="{{$dynamic_field->field_type}}"><?php echo ucfirst($dynamic_field->field_type);?></option>'
											  @endforeach
											+'</select>'
	...
	{ 
		...
		var url = "{{url('admin/questions/updated')}}";
		$('#Servicefrm').attr('action', url);
	
		var host="{{ url('admin/questions/get') }}";
									...
									+'<td>'
										+'<div class="col-sm-5">'
											...
											+'<select class="question_input form-control" name="question_input'+ s1x +'" data-sno="'+ s1x +'">'
											  ...
											  var selected = '';
											  if(data.dynamic_field_id == {{$dynamic_field->id}})
											  	selected = ' selected';
												
											  divbox += '<option value="{{$dynamic_field->field_type}}"'+selected+'><?php echo ucfirst($dynamic_field->field_type);?></option>'
											  @endforeach
											+'</select>'
										+'</div>'
										+'<div class="col-sm-3">Mandatory fields</div>';
	...
	{ 
		...
		var host="{{ url('admin/questions/question_detail_deleted') }}";
...
function deleterow(){ 
	if (confirm("Are you sure want to delete question?")) {
		var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
		var id= $(this).data('id'); 
		var host="{{ url('admin/questions/deleted/') }}";
		$.ajax({
			type: 'POST',
			data:{'id': id,'_token':CSRF_TOKEN},
			url: host,
