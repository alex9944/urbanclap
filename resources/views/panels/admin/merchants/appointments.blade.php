	...
	<div class="row">
					...
					<div class="x_panel">-->
						...
						<div class="col-md-5 col-xs-12">
							<div class="x_panel">
								<form name="actionForm" action="{{url('admin/merchants/appointments/destroy_all')}}" method="post" onsubmit="return deleteConfirm();"/> 
								<h2>Listing <span class="pull-right"><a href="{{url('admin/merchants/appointments')}}" class="btn btn-success btn-xs add_blog"><i class="fa fa-plus"></i> Add </a>  
								...
								<table id="datatable-checkbox" class="table table-striped table-bordered bulk_action">
									...
									<tbody>
										...
										<tr class="rm{{ $appointment_booking_setting->id }}">
											...
											<td>{{ @$listing->title }}</td>
											<td>@if($appointment_booking_setting->status == 1) Enable @else Disable @endif</td>
											<td>
						...
						<div class="col-md-7 col-xs-12">
							<div class="x_panel" id="add_div" style="">
								<?php
								$url = url('admin/merchants/appointments/add');
								$add = 'Add';
								if(old('id'))
								{
									$url = url('admin/merchants/appointments/update');
									$add = 'Edit';
								}
								?>
...
function edit_blogs(){ 
	...
	var host = "{{ url('admin/merchants/appointments') }}" + '/' + id + '/edit';
	var update_url = "{{url('admin/merchants/appointments/update')}}";
...
function deleteRow(){ 
	if (confirm("Are you sure want to delete the appointment booking?")) {
		...
		var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
		var id= $(this).attr('id'); 
		var host = "{{ url('admin/merchants/appointments') }}" + '/' + id;
		$.ajax({
			type: 'POST',
			...
			success: function(res)
			{
				if(res.success)
				{
					window.location = "{{ url('admin/merchants/appointments') }}";
				}
				else
				{
					alert(res.msg);
