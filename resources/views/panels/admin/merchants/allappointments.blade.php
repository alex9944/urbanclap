			...
			<div class="col-md-12 col-xs-12">
				<div class="x_panel">
					...
					<form name="actionForm" action="{{url('admin/services/destroy_all')}}" method="post"> 
						...
						<table id="all-appointments" class="table table-striped table-bordered bulk_action">
							<thead>
								<tr>
									<th>User Name</th>
									<th>Listing Name</th>					 
...
function enable(){ 
	...
	var id= $(this).attr('id'); 
	var host="{{ url('admin/merchants/appointments/enable') }}";
	$.ajax({
		type: 'POST',
...
function disable(){ 
	var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
	var id= $(this).attr('id'); 
	var host="{{ url('admin/merchants/appointments/disable') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id, '_token':CSRF_TOKEN},
		url: host,
