            ...
            <div class="row">
					...
					<!-- <div class="col-md-12 col-xs-12">
					 ...
					 <div class="col-md-5 col-xs-12">
								<div class="x_panel">
								<form name="actionForm" action="{{url('admin/multilanguage/destroy')}}" method="post" onsubmit="return deleteConfirm();"/> 
									<h2>All MultiLanguage <span class="pull-right"><a href="#" class="btn btn-success btn-xs add_blog"><i class="fa fa-plus"></i> Add </a>  
					...
					@foreach ($multilanguage as $multilanguage)
                        <tr class="rm{{ $multilanguage->id }}">
                          <td>
						  <input type="checkbox" name="selected_id[]" class="checkbox" value="{{ $multilanguage->id }}"/>				 	  
						  </td>
                          <td>{{ $multilanguage->title }}</td>
                         <td>
						 <!--<a href="#" class="btn btn-primary btn-xs"><i class="fa fa-folder"></i> View </a>-->
                            <a href="javascript:void(0);" class="edit_blog btn btn-info btn-xs" id="{{ $multilanguage->id }}" ><i class="fa fa-pencil"></i> Edit </a>
                            <a href="javascript:void(0);" class="btn btn-danger btn-xs delete_blog" id="{{ $multilanguage->id }}"><i class="fa fa-trash-o"></i> Delete </a></td>
                        </tr> 
					@endforeach
					...
					<div class="x_panel" id="add_div" style="">
						<h2>Add MultiLanguage </h2>
						<div class="x_title">
						</div>
						@if(Session::get('message')) <div class="alert alert-success" role="alert">{{Session::get('message')}} </div>@endif 
						<div class="alert alert-success hidden"></div>
								  <!-- Add Form Start-->
						 <form method="POST" action="{{url('admin/multilanguage/added')}}" accept-charset="UTF-8" enctype="multipart/form-data" class="form-horizontal">
					...
					<div class="x_panel" id="edit_div" style=" display:none">
						<h2>Edit MultiLanguage </h2>
						<div class="x_title">
						</div>					
								  <!-- Edit Form Start-->
						 <form method="POST" action="{{url('admin/multilanguage/updated')}}"  enctype="multipart/form-data" class="form-horizontal">
...
// EDit Blog
 $(document).on("click", ".edit_blog", edit_blogs);
   ...
   var id= $(this).attr('id'); 
	...
	var host="{{ url('admin/multilanguage/getmultilanguage/') }}";
	$('#add_div').hide();
...
function renderListform(res){ 
 ...
 $(document).on("click", ".delete_blog", deleteblog);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('admin/multilanguage/deleted/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
...
function deleteStatus(res){ 
 if (confirm("Are you sure delete multilanguage?")) {
...
function deleteConfirm(){
	if($('.checkbox:checked').length == ''){
		alert('Please check atleast one multilanguage');
		return false;
	}	
}
