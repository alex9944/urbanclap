            ...
            <div class="row">
					...
					<!-- <div class="col-md-12 col-xs-12">
					 ...
					 <div class="col-md-5 col-xs-12">
								<div class="x_panel">
								<form name="actionForm" action="{{url('merchant/campaigns/destroy')}}" method="post" onsubmit="return deleteConfirm();"/> 
									<h2>All Campaigns <span class="pull-right"><a href="{{url('merchant/campaigns')}}" class="btn btn-success btn-xs add_blog"><i class="fa fa-plus"></i> Add </a>  
					...
					<div class="x_panel" id="add_div" style="">
						...
						<div class="alert alert-success hidden"></div>
								  <!-- Add Form Start-->
						 <form method="POST" action="{{url('merchant/campaigns/added')}}" accept-charset="UTF-8" enctype="multipart/form-data" class="form-horizontal">
					...
					<div class="x_panel" id="edit_div" style=" display:none">
						...
						<div class="x_title">
								  ...
								  <!-- Edit Form Start-->
						 <form method="POST" action="{{url('merchant/campaigns/updated')}}"  enctype="multipart/form-data" class="form-horizontal">
 ...
 $(document).on("click", ".edit_blog", edit_blogs);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	...
	var host="{{ url('merchant/campaigns/getcampaigns/') }}";
	$('#add_div').hide();
...
function deleteblog(){ 
	if (confirm("Are you sure delete campaigns?")) 
	{
		var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
		var id= $(this).attr('id'); 
		var host="{{ url('merchant/campaigns/deleted/') }}";
		$.ajax({
			type: 'POST',
			data:{'id': id,'_token':CSRF_TOKEN},
			url: host,
...
//Change Status Enable
 ...
 $(document).on("click", ".enable", enable);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('merchant/campaigns/enable/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
...
//Change Status Disable
 ...
 $(document).on("click", ".disable", disable);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('merchant/campaigns/disable/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
 ...
 $(document).on("change", ".category", getcategory);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).val(); 
	...
	var host="{{ url('merchant/campaigns/getsubcategory/') }}";	
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
 ...
 $(document).on("change", ".country", getstates);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).val(); 
	...
	var host="{{ url('merchant/campaigns/getstates/') }}";	
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
 ...
 $(document).on("change", ".states", getcities);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).val(); 
	...
	var host="{{ url('merchant/campaigns/getcities/') }}";	
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
