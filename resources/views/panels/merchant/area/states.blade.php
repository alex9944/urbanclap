            ...
            <div class="row">
					...
					<!-- <div class="col-md-12 col-xs-12">
					 ...
					 <div class="col-md-5 col-xs-12">
								<div class="x_panel">
								<form name="actionForm" action="{{url('admin/states/destroy')}}" method="post" onsubmit="return deleteConfirm();"/> 
									<h2>All States <span class="pull-right"><a href="#" class="btn btn-success btn-xs add_blog"><i class="fa fa-plus"></i> Add </a>  
					...
					@foreach ($states as $states)
                        <tr class="rm{{ $states->id }}">
                          <td>
						  <input type="checkbox" name="selected_id[]" class="checkbox" value="{{ $states->id }}"/>				 	  
						  </td>
                          <td>{{ $states->name }}</td>
                         <td>
						 
						 <!--<a href="#" class="btn btn-primary btn-xs"><i class="fa fa-folder"></i> View </a>-->
                            <a href="javascript:void(0);" class="edit_blog btn btn-info btn-xs" id="{{ $states->id }}" ><i class="fa fa-pencil"></i> Edit </a>
                             <!--<a href="javascript:void(0);" class="btn btn-danger btn-xs delete_blog" id="{{ $states->id }}"><i class="fa fa-trash-o"></i> Delete </a>--></td>
                        </tr> 
					@endforeach
					...
					<div class="x_panel" id="edit_div" style=" display:none">
						...
						<div class="x_title">
								  ...
								  <!-- Edit Form Start-->
						 <form method="POST" action="{{url('admin/states/updated')}}"  enctype="multipart/form-data" class="form-horizontal">
 ...
 $(document).on("click", ".edit_blog", edit_blogs);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	...
	var host="{{ url('admin/states/getstates/') }}";
	$('#add_div').hide();
...
function renderListform(res){ 
 ...
 $(document).on("click", ".delete_blog", deleteblog);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('admin/states/deleted/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
...
function deleteStatus(res){ 
 if (confirm("Are you sure delete states?")) {
...
function deleteConfirm(){
	if($('.checkbox:checked').length == ''){
		alert('Please check atleast one state');
		return false;
	}	
}
