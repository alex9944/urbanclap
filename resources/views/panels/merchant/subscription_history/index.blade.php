            ...
            <div class="row">
					...
					<!-- <div class="col-md-12 col-xs-12">
					 ...
					 <div class="col-md-5 col-xs-12">
								<div class="x_panel">
								<form name="actionForm" action="{{url('merchant/subscription-history')}}" method="post" onsubmit="return deleteConfirm();"/> 
...
function deleteblog(){ 
	...
	if (confirm("Are you sure delete subscriber detail?")) 
	{
		var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
		var id= $(this).attr('id'); 
		var host="{{ url('merchant/subscription-history/deleted/') }}";
		$.ajax({
			type: 'POST',
			data:{'id': id,'_token':CSRF_TOKEN},
			url: host,
			...
			success: function(res)
			{
				if(res.success)
				{
					window.location = "{{ url('merchant/subscription-history') }}";
				}
				else
				{
					alert(res.msg);
