...
class ReviewsController extends Controller
{
	...
	$users = DB::table('listing_review')
		->select('listing_review.user_id','users.id','users.first_name','users.email')
	...
	public function show(Request $request)
		{
			 ...
			 $id=$request->id;  
			 $data['reviewsdetails'] = ListingReview::find($id);
	...
	public function listingReview(Request $request){
		...
		$notification->message=$request->name.' has added a review for '.$reviewobj->listing->title;
		$notification->listing_id=$request->listing_id;
		$notification->merchant_id=$request->merchant_id;
		$notification->save();*/
		
		// get all reviews
		$reviews = ListingReview::with('user')->where('listing_id', $request->listing_id)->ordered();
		
		$avgrating=DB::table('listing_review')->where('listing_id',$request->listing_id)->avg('rating');

		if(sizeof($reviews)>0){

			$excellentcount=DB::table('listing_review')->where('listing_id',$request->listing_id)->where('rating', 5)->where('approved','1')->count();
			$goodcount=DB::table('listing_review')->where('listing_id',$request->listing_id)->where('rating','<', 5)->where('rating','>=', 4)->where('approved','1')->count();
			$averagecount=DB::table('listing_review')->where('listing_id',$request->listing_id)->where('rating','<', 4)->where('rating','>=', '2.5')->where('approved','1')->count();
			$badcount=DB::table('listing_review')->where('listing_id',$request->listing_id)->where('rating','<', '2.5')->where('approved', 1)->count();
	...
	public function getReviews(){
		...
		$allreviews=ListingReview::get()->where('merchant_id',$user_id);
	...
	public function approveReview(Request $request){
		$listingreview = ListingReview::find($request->id);
